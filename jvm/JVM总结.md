...
内容较多，参看jvm规范或《深入理解Java虚拟机》

### 2.字节码指令

Oracle 提供了javap来反编译class文件 
...
![image-20200817202330273](JVM总结.assets/image-20200817202330273.png)
...
- 将类的字节码载入方法区中，内部采用C++的instanceKlass描述java类，它的重要属性有
...
- 加载和链接可能是交替运行的
  ...
  > **注意**
  >
  > - instantceKalss 这样的【元数据】是存储在方法区(1.8后的元空间内)，但java_mirror 是存储在堆中
  > - 可以通过HSDB工具查看
  >
  > ![image-20200817203336957](JVM总结.assets/image-20200817203336957.png)
...
用UE等支持二进制的编辑器修改HelloWorld.class 的信息，可在控制台中看到相关的错误信息

##### 准备

为static 变量分配空间，设置默认值

- static 变量在JDK7之前存储于instanceKlass 末尾，从JDK7开始，存储于_java_mirror 末尾
- static 变量分配空间和赋值是两个步骤，分配空间在准备阶段完成，赋值在初始化阶段完成
-  如果static 变量是final的基本类型，以及字符串常量，那么编译阶段就确定了，赋值在准备阶段完成
- 如果static 变量是final的，但属于引用类型，那么赋值会在初始化阶段完成
...
- 访问类的static final 表态变量(基本类型和字符串)不会触发初始化
- 类对象.class 不会触发初始化
- 创建该类的数组不会触发初始化
...
- 其中`/a:.`表示将当前目录追加到bootclasspath之后
- 可以用这个办法替换核心类
...
使用线程上下文类加载器可以打破双亲委派模型的层次结构来逆向使用类加载器
...
>
>    注意不是重写loadClass 方法，否则不会走双亲委派机制
>
> 3. 读取类文件的字节码
>
...
> profiling 是指在运行过程中收集一些程序执行状态的数据，使用【方法的调用次数】，【循环的回边次数】等
...
对于占据大部分的不常用的代码，我们无需耗费时间将其编译成机器码，而是采用解释执行的方式运行；另一方面，对于仅占据小部分的热点代码，我们则可以将其编译成机器码，以达到理想的运行速度。执行效率上简单比较一下Interpreter < C1 < C2，总的目标是发现热点代码(hostspot 名称的由来)
**...等优化手段**参看《深入理解Java虚拟机》
